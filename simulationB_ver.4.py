...
# ノードBからのUWBによる角度と距離をずらして計算
angle_bd_shifted = angle_b + np.radians(5)  # 5度ずらす例
distance_bd_shifted = distance_b

# ベクトル合成してノードDの位置を計算
vector_ad = np.array([distance_cd * np.cos(angle_cd) + distance_bd_shifted * np.cos(angle_bd_shifted),
                      distance_cd * np.sin(angle_cd) + distance_bd_shifted * np.sin(angle_bd_shifted)])
...
plt.scatter(*node_a_position, label='Node A (True)', marker='o', color='red',s=100)
plt.scatter(*node_b_position, label='Node B (Unknown)', marker='o', color='blue',s=100)
plt.scatter(*node_c_position, label='Node C (Unknown)', marker='o', color='green',s=100)
plt.scatter(*true_node_d_position, label='Node D (True)', marker='o', color='purple',s=100)
...
# プロットの設定
plt.title('Node Positions and Estimates with Shifted Node B',fontsize=25)
